...
const routeMatchesItem = computed(() => {
    if (!props.item.to) {
        return false;
    }
...
const isActive = computed(() => {
    return props.item.path ? layoutState.activePath?.startsWith(fullPath.value) : routeMatchesItem.value || layoutState.activePath === props.item.to;
});
