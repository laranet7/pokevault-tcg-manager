...
const GUEST_LAYOUT_KEY = 'pokevault:layout:guest';
...
function getLayoutStorageKey(userId = getStoredUserId()) {
    return userId ? `pokevault:layout:user:${userId}` : GUEST_LAYOUT_KEY;
}

function getPrimaryColor(name) {
    return primaryColors.find((color) => color.name === name) ?? primaryColors[1];
}

function getSurface(name) {
    return surfaces.find((surface) => surface.name === name) ?? surfaces[0];
}

function getPresetExt() {
    const color = getPrimaryColor(layoutConfig.primary);
...
function persistLayoutPreferences(userId = getStoredUserId()) {
    if (typeof window === 'undefined') {
        return;
    }
    ...
    window.localStorage.setItem(getLayoutStorageKey(userId), JSON.stringify(snapshot));
}

function applyTheme() {
    const presetTheme = presets[layoutConfig.preset] ?? Aura;
    const surfacePalette = getSurface(layoutConfig.surface).palette;

    $t().preset(presetTheme).preset(getPresetExt()).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
    document.documentElement.classList.toggle('app-dark', layoutConfig.darkTheme);
}

function loadLayoutPreferences(userId = getStoredUserId()) {
    if (typeof window === 'undefined') {
        return { ...defaultLayoutConfig };
    }

    const userConfig = safeParse(window.localStorage.getItem(getLayoutStorageKey(userId)));
    const guestConfig = safeParse(window.localStorage.getItem(GUEST_LAYOUT_KEY));
    return { ...defaultLayoutConfig, ...(guestConfig ?? {}), ...(userConfig ?? {}) };
}

export function applyLayoutPreferencesForUser(userId = getStoredUserId()) {
    const snapshot = loadLayoutPreferences(userId);

    layoutConfig.preset = snapshot.preset;
    layoutConfig.primary = snapshot.primary;
    layoutConfig.surface = snapshot.surface;
    layoutConfig.darkTheme = snapshot.darkTheme;
    layoutConfig.menuMode = snapshot.menuMode;

    applyTheme();
    persistLayoutPreferences(userId);
}

export function initializeLayout() {
    applyLayoutPreferencesForUser();
}
...
export function useLayout() {
    ...
    const changeMenuMode = (eventOrValue) => {
        layoutConfig.menuMode = eventOrValue?.value ?? eventOrValue;
...
export { menuModeOptions, presetOptions, presets, primaryColors, surfaces };
